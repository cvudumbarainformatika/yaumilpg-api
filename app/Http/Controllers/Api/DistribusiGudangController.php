...
class DistribusiGudangController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = PengeluaranGudang::query()
        ->select('pengeluaran_gudangs.*','users.name as user_name')
        ->leftJoin('users', 'pengeluaran_gudangs.user_id', '=', 'users.id')
        ->with(['items.product.satuan']);
        ...
        if ($request->filled('q') && !empty($request->q)) {
            ...
            $query->where(function($q) use ($search) {
                $q->where('pengeluaran_gudangs.reference_no', 'like', "%{$search}%")
        ...
        if ($request->filled('start_date') && $request->filled('end_date')) {
            $query->whereBetween('pengeluaran_gudangs.out_at', [
        ...
        $returns = $query->latest('pengeluaran_gudangs.created_at')->simplePaginate($perPage);
    ...
    public function store(Request $request)
    {
        ...
        try {
            $kode = 'DG-' . now()->format('Ymd') . '-' . strtoupper(Str::random(4));
            // $total = collect($data['items'])->sum(fn($i) => $i['qty'] * $i['harga']);

            $dist = PengeluaranGudang::create([
                'reference_no' => $kode,
                'user_id' => Auth::id(),
                'out_at' => $data['out_at'],
            ...
            DB::commit();
            return response()->json(['message' => 'Distribusi berhasil disimpan', 'data' => $dist->load('items.product')]);
        } catch (\Throwable $e) {
            DB::rollBack();
            return response()->json(['message' => 'Gagal menyimpan Distribusi gudang', 'error' => $e->getMessage()], 500);
        }
    }
